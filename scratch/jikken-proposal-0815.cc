      ...
      for (uint32_t i = 0; i < table.size (); ++i)
        {
          ...
          std::cout << "dest is " << route.destAddr << ", pred is " << route.preds[convarray_node_dtnaddr[recv]] << ", interface is " << route.interface << std::endl;
          ...
          if (listOlsr)
            {
              ...
              std::cout << "table2.size() is " << table2.size() << std::endl;
              for (uint32_t i = 0; i < table2.size (); ++i)
                {
                  olsr::RoutingTableEntry route2 = table2[i];
                  std::cout << "dest is " << route2.destAddr << ", pred is " << route2.preds[convarray_node_dtnaddr[recv]] << ", interface is " << route2.interface << std::endl;
      ...
      if (table_addr[i] == Ipv4Address ("102.102.102.102"))
        {
          ...
          if (sender != recv)
          {
              ...
              std::cout << convarray[convarray_node_olsraddr[sender]] << " to " << convarray[convarray_node_olsraddr[recv]] << " pred is " << s_pred << std::endl;

              // 経路表の宛先到達確率 > 自分の宛先到達確率となる、IPアドレスを持つノードを抽出する
              for (auto i = ipandpred.begin(); i != ipandpred.end(); ++i)
              {
                  std::cout << "i->second is " << i->second << std::endl;
                  //std::cout << "2" << std::endl;
                  if (s_pred < i->second)
                  {
...
main (int argc, char *argv[])
{
  ...
  std::ifstream ifss ("./random-waypoint-mobility2.mob");
  std::string str;
  if (ifss.fail ())
    {
