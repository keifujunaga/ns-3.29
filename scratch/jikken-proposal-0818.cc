...
void olsr_testApp (Ptr<const Packet> packet, int recv, int sender, int inter, int mode, int uid, bool promode);
...
public:
  ...
  void Setup (Ptr<const Packet> packet, Ptr<Socket> socket, Address address, uint32_t senderAdress, uint32_t receiverAddress, uint32_t interAddress, 
...
void
MyApp::Setup (Ptr<const Packet> packet, Ptr<Socket> socket, Address address, uint32_t sender, uint32_t receiver, uint32_t inter, 
              uint32_t packetSize, uint32_t nPackets, uint32_t mode, int uid, DataRate dataRate)
{
  m_packet = packet;
  m_socket = socket;
...
MyApp::SendPacket (void)
{
  ...
  if (!(m_sender == m_receiver)) 
      ...
      if (m_uid < 0) // This packet is generated by sender
        {
          if (start_mem[m_uid * 10 + m_sender][m_receiver] == false)
          {
            NS_LOG_INFO("(MyApp::SendPacket) Time:, " << Simulator::Now().GetSeconds() << ", sender, " << m_sender << ", send packet, " << new_packet->GetUid() << ", to receiver, " << m_receiver <<  ", mode:, " << m_mode << ", is started");
            send_count[new_packet->GetUid()] = true;
            tag.SetToken (new_packet->GetUid () * 100000 + m_receiver * 1000 + m_inter * 10 + m_mode);
            new_packet->AddPacketTag (tag);
            m_socket->Send (new_packet);
            start_mem[m_uid * 10 + m_sender][m_receiver] = true;
          }
        }
      else // This packet is generated by intermediary node
        {
          if (mem[m_uid * 10 + m_sender][m_receiver] == false) 
          {
            ...
            std::cout << "(MyApp::SendPacket) Time:, " << Simulator::Now().GetSeconds() << ", m_packet->Copy() uid is, " << old_packet->GetUid() << ", old_packet's tag token is, " << token << std::endl;
...
IncRate (Ptr<MyApp> app, DataRate rate)
{
 ...
 CourseChange (std::string context, Ptr<const MobilityModel> position)
 {
  ...
  NS_LOG_INFO("(CourseChange), " << Simulator::Now () << ", kym=, " << kym << ", pos=, " << position << ", x=, " << pos.x << ", y=, " << pos.y << ", z=, " << pos.z);
 }
      ...
      if (table_addr[i] == Ipv4Address ("102.102.102.102"))
        {
          ...
          if (sender != recv)
          {
              ...
              std::cout << "(do_proposal) time, " << Simulator::Now().GetSeconds()  << ", recv, " << recv << ", sender, " << sender << ", pred is, " << s_pred << ", ";
              ...
              for (auto i = ipandpred.begin(); i != ipandpred.end(); ++i)
              {
                  ...
                  if (s_pred < i->second)
                  {
                      ...
                      std::cout << "(do_proposal2) time, " << Simulator::Now().GetSeconds() << ", s_pred, " << s_pred << ", dest's pred, " << i->second << ", is s_pred < dest's pred, " << ", then send packet, " << packet->GetUid() << ", is send to the Node, " << convarray_olsraddr_node[i->first] << ", (Address is, " <<  i->first << ", )" << std::endl;
                      olsr_testApp(packet, recv, sender, convarray_olsraddr_node[i->first], mode, uid, true);
                  }
              }
              std::cout << std::endl;
              ...
              {
                // DTNでパケットを送信する
                std::cout << "(do_proposal3) time, " << Simulator::Now().GetSeconds() << ", " << "do_dtntestapp" << std::endl;
  ...
  if (packet->GetSize () >= packetSize)
    {
      ...
      if (mode == 1)
      {
        if (kym >= numberOfueNodes)
        {
          NS_LOG_INFO("(Ipv4L3Protocol/Rx), " << Simulator::Now().GetSeconds() << ", " << kym);
          kym = kym - numberOfueNodes;
          if (kym != (int) recv) // if node is relay node try to connect to OLSR
            {
              NS_LOG_INFO("(Ipv4L3Protocol/Rx), " << Simulator::Now().GetSeconds() << ", do dtn_to_olsr");
              int time = Simulator::Now ().GetSeconds ();
              int endtime = (int) appDataEnd - time;
              for (int i = 0; i < endtime; i++) 
                {
                  Simulator::Schedule (Seconds (i), &dtn_to_olsr, packet, kym, recv, inter, uid);
                }
            }
        }
      } 
...
void
dtn_to_olsr (Ptr<const Packet> packet, int sender, int recv, int inter, int uid)
{
  Ptr<Node> node = olsrContainer.Get (recv);
  Ptr<Ipv4> ipv4 = node->GetObject<Ipv4> ();
  Ipv4Address recvaddr = ipv4->GetAddress (1, 0).GetLocal ();
...
void
olsr_testApp (Ptr<const Packet> packet, int recv, int sender, int inter, int mode, int uid, bool promode)
{
  int numPackets = 1;
  if (promode == false) 
  {
    ...
    app1->Setup (packet, ns3UdpSocket1, sinkAddress1, sender, recv, inter, packetSize, numPackets, mode, uid, DataRate ("1Mbps"));
...
dtn_testApp (Ptr<const Packet> packet, int recv, int sender, int inter, int mode, int uid)
{
  ...
  app2->Setup (packet, ns3UdpSocket2, sinkAddress2, sender, recv, inter, packetSize, numPackets, mode, uid,
               DataRate ("1Mbps"));
...
UdpL4ProtocolDrop (std::string context, Ptr<const Packet> packet)
{
  ...
  NS_LOG_INFO ("(UdpL4Protocol/Drop), " << Simulator::Now ().GetSeconds () << ", " << kym << ", "
                                      << packet->GetUid () << ", " << uid << ", " <<  packet->GetSize());
}
...
void
Ipv4L3Drop (std::string context, const Ipv4Header & header, Ptr<const Packet> packet, Ipv4L3Protocol::DropReason reason, Ptr<Ipv4> ipv4, uint32_t interface)
{
  std::string delim ("/");
  std::list<std::string> list_string;
  boost::split (list_string, context, boost::is_any_of (delim));
  ...
  NS_LOG_INFO("(Ipv4L3Protocol/Drop), " << Simulator::Now().GetSeconds() << ", " 
                              << kym << ", " << packet->GetUid() << ", " << uid << ", " << packet->GetSize() 
                              << ", " << reason);
}
...
ArpCacheDrop (std::string context, Ptr<const Packet> packet)
{
  ...
  NS_LOG_INFO("(ArpCache/Drop), " << Simulator::Now().GetSeconds() << ", " << kym << ", " << packet->GetUid() << ", " << uid << ", " << packet->GetSize());
}
...
main (int argc, char *argv[])
{
  ...
  std::ifstream ifss ("./random-waypoint-mobility0901.mob");
  std::string str;
  if (ifss.fail ())
    {
      ...
      for (int v = 0; v < jc[u-numberOfvehicles]; v++)
        {
          waypoint->AddWaypoint (
              Waypoint (NanoSeconds (wpl[u-numberOfvehicles][v].waypoint_t),
                        Vector (wpl[u-numberOfvehicles][v].waypoint_x, wpl[u-numberOfvehicles][v].waypoint_y, 1.5)));
          waypoint2->AddWaypoint (
              Waypoint (NanoSeconds (wpl[u-numberOfvehicles][v].waypoint_t),
                        Vector (wpl[u-numberOfvehicles][v].waypoint_x, wpl[u-numberOfvehicles][v].waypoint_y, 1.5)));
        }
    }
 ...
 */
  ...
  */
  ...
  Config::Connect("NodeList/*/$ns3::UdpL4Protocol/SocketList/*/Drop", MakeCallback(&UdpL4ProtocolDrop));
