...
main (int argc, char** argv)
{
  ...
  double appDataEnd = 1000.0;
  double appTotalTime = appDataStart + appDataEnd; // simulator stop time

  //Create node container

  CommandLine cmd;
  cmd.AddValue ("randnum", "random value for RNG", randnum);
  cmd.Parse(argc, argv);

  NS_LOG_INFO("randnum value is " << randnum);
  std::cout << "randnum value is " << randnum << std::endl;
  ...
  std::srand (randnum);
  RngSeedManager::SetSeed(randnum);
  ...
  */
  ...
  std::string filename = "random-waypoint-mobility-" + str + ".mob";
