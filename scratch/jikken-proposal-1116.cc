...
// OLSRで宛先までパケットを送信した回数を記録する配列
std::map<int, int> olsr_sender_mem;
std::map<int, int> intv_sender_count;
std::map<int, int> range_count;

std::map<int, int> olsr_sender_mem_recv;
std::map<int, int> intv_sender_count_recv;
std::map<int, int> range_count_recv;
      ...
      if (m_uid < 0) // This packet is generated by sender
        {
          if (start_mem[m_sender][m_uid * 10 + m_receiver] == false)
            {
              NS_LOG_INFO ("(MyApp::SendPacket) Time:, "
                           << Simulator::Now ().GetSeconds () << ", sender, " << m_sender
                           << ", send packet, " << new_packet->GetUid () << ", to inter, "
                           << m_inter << ", to receiver, " << m_receiver << ", mode:, " << m_mode
                           << ", is started");
          ...
          if (mem[m_sender][m_uid * 10 + m_receiver] == false)
            {
              ...
              std::cout << "(MyApp::SendPacket) Time:, " << Simulator::Now ().GetSeconds ()
                        << ", sender, " << m_sender << ", m_packet->Copy() uid is, "
                        << old_packet->GetUid () << ", to inter, " << m_inter << ", to receiver, "
                        << m_receiver << ", mode:, " << m_mode << ", old_packet's tag token is, "
                        << token << std::endl;
...
CourseChange (std::string context, Ptr<const MobilityModel> position)
{
  ...
  NS_LOG_INFO ("(CourseChange), " << Simulator::Now ().GetSeconds () << ", kym=, " << kym
                                  << ", pos=, " << position << ", x=, " << pos.x << ", y=, "
                                  << pos.y << ", z=, " << pos.z);
  double distance;
  if (6 <= kym && kym <= 8)
    {
      ...
      NS_LOG_INFO ("(CourseChange2), " << Simulator::Now ().GetSeconds () << ", kym=, " << kym
                                       << ", distance=, " << distance);
    }
}
...
TraceupdateTransitivePreds (std::string context, Ipv4Address addr)
{
  ...
  if (kym >= total_node_num)
    kym = kym - total_node_num;
...
TraceSendPacketFromQueue (std::string context, Ipv4Address dst, Ptr<const Packet> packet)
{
  ...
  NS_LOG_INFO ("(TraceSendPacketFromQueue), uid, " << uid << ", recv, " << recv << ", inter, "
                                                   << inter << ", mode, " << mode);
  ...
  if (main_pastcontact[address][address2] == true && main_pastcontact[address2][address3] == true)
    {
      ...
      Ptr<Node> node = dtnContainer.Get (kym - total_node_num);
      Ptr<Ipv4> ipv4 = node->GetObject<Ipv4> ();
      Ptr<Ipv4RoutingProtocol> proto = ipv4->GetRoutingProtocol ();
      Ptr<Epidemic::RoutingProtocol> epi = DynamicCast<Epidemic::RoutingProtocol> (proto);
  ...
  if (0 < token && token < 947000000000)
    {
      if (packet->GetSize () > packetSize)
        {
          NS_LOG_INFO ("(TraceRouteInput), "
                       << "time, " << Simulator::Now ().GetSeconds () << ", kym, " << kym
                       << ", token, " << token << ", uid, " << uid << ", recv, " << recv
                       << ", inter, " << inter << ", packetsize, " << packet->GetSize ());
...
olsr_or_dtn (int sender, int recv, int uid)
{
  int mode = 0;
  int inter = 99;
  Ipv4Address table_addr[total_node_num];
  ...
  /// ここで配列を作る
  // sender_mem[uid][recv] += 1;

  intv_sender_count[uid] += 1;
  intv_sender_count_recv[recv] += 1;
  ...
  if (recv == 6)
    {
      distance =
          sqrt (pow (mob->GetPosition ().x - 500, 2) + pow (mob->GetPosition ().y - 1500, 2));
      std::cout << "recv6, mob->GetPosition().x, " << mob->GetPosition ().x
                << ", mob->GetPosition().y, " << mob->GetPosition ().y << std::endl;
    }
  else if (recv == 7)
    {
      distance =
          sqrt (pow (mob->GetPosition ().x - 1500, 2) + pow (mob->GetPosition ().y - 500, 2));
      std::cout << "recv7, mob->GetPosition().x, " << mob->GetPosition ().x
                << ", mob->GetPosition().y, " << mob->GetPosition ().y << std::endl;
    }
  else if (recv == 8)
    {
      distance =
          sqrt (pow (mob->GetPosition ().x - 2500, 2) + pow (mob->GetPosition ().y - 1500, 2));
      std::cout << "recv8, mob->GetPosition().x, " << mob->GetPosition ().x
                << ", mob->GetPosition().y, " << mob->GetPosition ().y << std::endl;
    }
  ...
  for (int i = 0; i < total_node_num; i++)
    {
      ...
      if (recvaddr == table_addr[i])
        {
          mode = 0;
          olsr_testApp (packet, recv, sender, inter, mode, uid,
                        false); /// ここでログをとれるようにする
          mem[sender][uid * 10 + recv] = true;
          olsr_sender_mem[uid] += 1;
          olsr_sender_mem_recv[recv] += 1;
          break;
        }
          ...
          if (sender != recv)
            {
              ...
              std::cout << std::endl;
              std::cout << "(do_proposal3), queuelist[" << sender << "][" << uid << "* 10 + "
                        << recv << "] = " << queuelist[sender][uid * 10 + recv] << std::endl;
              std::cout << "(do_proposal3), mem[" << sender << "][" << uid << "* 10 + " << recv
                        << "] = " << mem[sender][uid * 10 + recv] << std::endl;
      ...
      if (6 <= recv && recv <= 8)
        {
          if (recv == 6)
            {
              distance = sqrt (pow (mob->GetPosition ().x - 500, 2) +
                               pow (mob->GetPosition ().y - 1500, 2));
              std::cout << "recv6, mob->GetPosition().x, " << mob->GetPosition ().x
                        << ", mob->GetPosition().y, " << mob->GetPosition ().y << std::endl;
            }
          else if (recv == 7)
            {
              distance = sqrt (pow (mob->GetPosition ().x - 1500, 2) +
                               pow (mob->GetPosition ().y - 500, 2));
              std::cout << "recv7, mob->GetPosition().x, " << mob->GetPosition ().x
                        << ", mob->GetPosition().y, " << mob->GetPosition ().y << std::endl;
            }
          else if (recv == 8)
            {
              distance = sqrt (pow (mob->GetPosition ().x - 2500, 2) +
                               pow (mob->GetPosition ().y - 1500, 2));
              std::cout << "recv8, mob->GetPosition().x, " << mob->GetPosition ().x
                        << ", mob->GetPosition().y, " << mob->GetPosition ().y << std::endl;
            }
...
{
  intv_uid--;
  for (int i = 6; i < total_node_num; i++)
    {
      for (int j = 6; j < total_node_num; j++)
        {
          if (i != j)
...
std::map<int, int> olsr_sender_mem_true;
std::map<int, int> olsr_sender_mem_false;

void
checkpacket ()
{
  for (auto itr = olsr_sender_mem.begin (); itr != olsr_sender_mem.end (); ++itr)
    {
      if (recv_count[itr->first] == true)
        {
          olsr_sender_mem_true[itr->first] = itr->second;
          NS_LOG_INFO ("(olsr_sender_mem_true), "
                       << "uid, " << itr->first << ", count, " << itr->second
                       << ", intv_sender_count, " << intv_sender_count[itr->first]
                       << ", range_count, " << range_count[itr->first]);
        }
      else
        {
          olsr_sender_mem_false[itr->first] = itr->second;
          NS_LOG_INFO ("(olsr_sender_mem_false), "
                       << "uid, " << itr->first << ", count, " << itr->second
                       << ", intv_sender_count, " << intv_sender_count[itr->first]
                       << ", range_count, " << range_count[itr->first]);
        }
...
main (int argc, char *argv[])
{
  ...
  // 一斉に充電場所を変える
  double uav_battery_time[] = {900, 900, 900, 900, 900, 900};
  int hovering_time = 900;
  int one_way_time = 360;
  int battery_change_time = 60;
  ...
  for (int i = 0; i < uav_num; i++)
    {
      j_count = 0; // reset
      for (double j = uav_battery_time[i]; j + one_way_time*2 + battery_change_time < sim_total_time; j = j + hovering_time)
        {
  ...
  for (int u = uav_num; u < total_node_num; u++)
    {
      ...
      for (int v = 0; v < jc[u - uav_num]; v++)
        {
          std::cout << "wpl[" << u-uav_num << "][" << v << "] is " <<wpl[u - uav_num][v].waypoint_t<< std::endl;
  ...
  *     Internet Stack Setup
  ...
  for (int i = 30; i < (int) sim_total_time; i = i + 30)
    {
      Simulator::Schedule (Seconds (i), &sendpacket_interval);
    }

  Simulator::Schedule (Seconds (appDataEnd-5), &checkpacket);
  ...
  */
  ...
  AnimationInterface anim ("jikken-proposal-1116.xml");
  //anim.EnablePacketMetadata (true);
  //anim.EnableIpv4RouteTracking ("tinkoko.xml", Seconds (0), Seconds (appDataEnd), Seconds (0.25));

  AsciiTraceHelper ascii;
  std::string filename = "jikken-proposal-1116.mob";
  MobilityHelper::EnableAsciiAll (ascii.CreateFileStream (filename));
