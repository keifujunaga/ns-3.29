...
{
  intv_uid--;
  for (int i = 3; i < numberOfueNodes; i++)
  {
    for (int j = 3; j < numberOfueNodes; j++)
    {
      if (i != j)
        olsr_or_dtn (i, j, intv_uid);
    }
  }
  NS_LOG_INFO("send_count is " << send_count.size() << ", recv_count is " << recv_count.size());
}
...
main (int argc, char *argv[])
{
  ...
  initialAlloc->Add (Vector (252, 185, 1.5));
  initialAlloc->Add (Vector (132, 150, 1.5));
  initialAlloc->Add (Vector (1623, 652, 1.5));
  initialAlloc->Add (Vector (1939, 920, 1.5));
  initialAlloc->Add (Vector (2382, 800, 1.5));
  initialAlloc->Add (Vector (2736, 573, 1.5));
  /*
  initialAlloc->Add (Vector (2593, 292, 1.5));
  initialAlloc->Add (Vector (2772, 193, 1.5));
  initialAlloc->Add (Vector (2679, 248, 1.5));
  */
  ...
  */
  ...
  for (int i = 3; i <= 5; i++)
  {
    mobility1.Install(olsrContainer.Get(i));
    mobility1.Install(dtnContainer.Get(i));
  }
  ...
  mobility2.SetMobilityModel ("ns3::RandomWalk2dMobilityModel", 
                              "Bounds", StringValue("0|2000|0|1000"),
                              "Speed", StringValue("ns3::UniformRandomVariable[Min=2.17|Max=2.17]"));
  */
  for (int i = 6; i <= 8; i++)
  {
 ...
 */
  ...
  //Simulator::Schedule (timer, &olsr_testApp, 1, 0, 0, -1);
  //Simulator::Schedule (timer, &olsr_testApp, 2, 1, mode, uid);

  // Simulator::Schedule (timer, &olsr_testApp, 4, 0, mode, uid);
