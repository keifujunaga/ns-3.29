...
main (int argc, char *argv[])
{
  ...
  */
  ...
  {
    if (i == NUM_UAV+2)   
      initialAlloc->Add (Vector ((i+1)*251, 0, 1.5));
    else
...
*/
  ...
  */
  ...
  oss3 << "/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Phy/PhyRxDrop";
  oss4 << "/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Phy/PhyTxDrop";
  oss5 << "/NodeList/*/$ns3::Ipv4L3Protocol/Drop";

  // Config::Connect (oss.str (), MakeCallback (&CourseChange));
  Config::Connect (oss2.str (), MakeCallback (&phyMonitorSniffRx));
  Config::Connect (oss3.str (), MakeCallback (&phyRxDrop));
  Config::Connect (oss4.str (), MakeCallback (&phyTxDrop));
  Config::Connect (oss5.str (), MakeCallback (&phyTxDrop));
