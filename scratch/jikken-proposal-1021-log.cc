...
olsr_or_dtn (int sender, int recv, int uid)
{
  ...
  if (recv == 6)
  {
    distance = sqrt(pow(mob->GetPosition().x - 500, 2) + pow(mob->GetPosition().y - 1500, 2));
    std::cout << "recv6, mob->GetPosition().x, " << mob->GetPosition().x << ", mob->GetPosition().y, " << mob->GetPosition().y << std::endl;
  }
  else if (recv == 7)
  {
    distance = sqrt(pow(mob->GetPosition().x - 1500, 2) + pow(mob->GetPosition().y - 500, 2));
    std::cout << "recv7, mob->GetPosition().x, " << mob->GetPosition().x << ", mob->GetPosition().y, " << mob->GetPosition().y << std::endl;
  }
  else if (recv == 8)
  {
    distance = sqrt(pow(mob->GetPosition().x - 2500, 2) + pow(mob->GetPosition().y - 1500, 2));
    std::cout << "recv8, mob->GetPosition().x, " << mob->GetPosition().x << ", mob->GetPosition().y, " << mob->GetPosition().y << std::endl;
  }
  ...
  for (int i = 0; i < numberOfueNodes; i++)
    {
      ...
      if (recvaddr == table_addr[i])
        {
          mode = 0;
          olsr_testApp (packet, recv, sender, inter, mode, uid, false); /// ここでログをとれるようにする
...
void checkpacket()
{
  for (auto itr = olsr_sender_mem.begin(); itr != olsr_sender_mem.end(); ++itr)
  {
    if (recv_count[itr->first] == true)
    {
      ...
      NS_LOG_INFO("(olsr_sender_mem_true), " << "uid, " << itr->first << ", count, " << itr->second << ", intv_sender_count, " << intv_sender_count[itr->first] << ", range_count, " << range_count[itr->first]);
    }
    else
    {
      olsr_sender_mem_false[itr->first] = itr->second;
      NS_LOG_INFO("(olsr_sender_mem_false), " << "uid, " << itr->first << ", count, " << itr->second << ", intv_sender_count, " << intv_sender_count[itr->first] << ", range_count, " << range_count[itr->first]);
    }
  }
}
 ...
 */
  ...
  Simulator::Schedule (Seconds(995), &checkpacket);
