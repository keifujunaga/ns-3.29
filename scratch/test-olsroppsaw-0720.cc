  ...
  if (mem[m_uid * 10 + m_sender][m_receiver] == false && !(m_sender == m_receiver)) // if this node has never sent a packet
    {
      ...
      NS_LOG_INFO("(MyApp::SendPacket) Time: " << Simulator::Now().GetSeconds() << " sender " << m_sender << " send packet " << packet->GetUid() << "to receiver " << m_receiver <<  " is started");
...
void
olsr_or_dtn (int sender, int recv)
{
  int mode = 0;
  int uid = -1;
      ...
      Ipv4L3Protocol::DropReason reason, Ptr<Ipv4> ipv4, uint32_t interface)
{
  ...
  std::cout << "kym is "<< kym << " (Ipv4L3Protocol/Drop) packet->GetSize() " << packet->GetSize () << "packet dest is " <<  header.GetDestination() << std::endl;
}
...
MacRxDrop (std::string context, Ptr<const Packet> packet)
{
  ...
  ++itr;
  int kym = std::stoi (*itr); // NodeNo
  std::cout << "kym is "<< kym << " (WifiMac/MaxRxDrop) packet->GetSize() " << packet->GetSize () << std::endl;
...
MacTxDrop (std::string context, Ptr<const Packet> packet)
{
  ...
  ++itr;
  int kym = std::stoi (*itr); // NodeNo
  std::cout << "kym is "<< kym << " (WifiMac/MacTxDrop) packet->GetSize() " << packet->GetSize () << std::endl;
}
...
PhyTxDrop (std::string context, Ptr<const Packet> packet)
{
  ...
  ++itr;
  int kym = std::stoi (*itr); // NodeNo
  std::cout << "kym is "<< kym << " (WifiPhy/PhyTxDrop) packet->GetSize() " << packet->GetSize () << std::endl;
}
...
PhyRxDrop (std::string context, Ptr<const Packet> packet)
{
  ...
  ++itr;
  int kym = std::stoi (*itr); // NodeNo
  if (packet->GetSize() >= 2000)
  {
    std::cout << "kym is "<< kym << " (WifiPhy/PhyRxDrop) packet->GetSize() " << packet->GetSize () << std::endl;
  }
}

void
TxErrHeader (std::string context, const WifiMacHeader &hdr)
{
  std::string delim ("/");
  std::list<std::string> list_string;
  boost::split (list_string, context, boost::is_any_of (delim));
  ...
  ++itr;
  int kym = std::stoi (*itr); // NodeNo
  std::cout << "(TxErrHeader) kym is "<< kym << std::endl;
}

void
TxOkHeader (std::string context, const WifiMacHeader &hdr)
{
  std::string delim ("/");
  std::list<std::string> list_string;
  boost::split (list_string, context, boost::is_any_of (delim));
  ...
  ++itr;
  int kym = std::stoi (*itr); // NodeNo
  std::cout << "(TxOkHeader) kym is "<< kym << std::endl;
}
 ...
 */
  ...
  makeconvarray ();
  makeconvarray_node_olsraddr ();
  //Simulator::Schedule (timer, &olsr_testApp, 2, sender, mode, uid);
  //Simulator::Schedule (timer, &olsr_testApp, 3, sender, mode, uid);
  //Simulator::Schedule (timer, &olsr_testApp, 4, sender, mode, uid);
  Simulator::Schedule (timer, &olsr_or_dtn, 0, 4);
  ...
  */
  ...
  //Config::Connect ("NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Phy/PhyRxDrop", MakeCallback (&PhyRxDrop));
  //Config::Connect ("NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::RegularWifiMac/TxErrHeader", MakeCallback (&TxErrHeader));
  //Config::Connect ("NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::RegularWifiMac/TxOkHeader", MakeCallback (&TxOkHeader));
