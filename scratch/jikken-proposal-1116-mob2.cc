...
main (int argc, char **argv)
{
  ...
  // 一斉に充電場所を変える
  // double uav_battery_time[] = {900, 900, 900, 900, 900, 900, 900, 900, 900, 900, 900, 900};
  ...
  for (int i = 0; i < uav_num; i++)
    {
      ...
      if (i < uav_num / 2)
        {
          // 3 → 4 → 1 → 2
          for (double j = one_way_time;
               j + battery_change_time + one_way_time + hovering_time < sim_total_time;
               j = j + one_way_time)
            {
              j_count++;
              waypoint_time[i][j_count] = j; // 6分
              std::cout << "waypointtime[" << i << "][" << j_count << "] is " << waypoint_time[i][j_count] << std::endl;

              j_count++;
              waypoint_time[i][j_count] = j + battery_change_time; // 6+1分
              std::cout << "waypointtime[" << i << "][" << j_count << "] is " << waypoint_time[i][j_count] << std::endl;
              j += battery_change_time;

              j_count++;
              waypoint_time[i][j_count] = j + one_way_time; // 6+1+6分
              std::cout << "waypointtime[" << i << "][" << j_count << "] is " << waypoint_time[i][j_count] << std::endl;
              j += one_way_time;

              j_count++;
              waypoint_time[i][j_count] = j + hovering_time; // 6+1+6+15分
              std::cout << "waypointtime[" << i << "][" << j_count << "] is " << waypoint_time[i][j_count] << std::endl;
              j += hovering_time;
            }
        }
  ...
  mobility3.SetMobilityModel("ns3::WaypointMobilityModel");
  //mobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");

  for (int i = 0; i < uav_num/2; i++)
    {
      mobility.Install (olsrContainer.Get (i));
      mobility.Install (dtnContainer.Get (i));
    }

  for (int i = uav_num/2; i < uav_num; i++)
  {
      mobility3.Install (olsrContainer.Get (i));
      mobility3.Install (dtnContainer.Get (i));
  }
  ...
  for (int i = 0; i < uav_num / 2; i++)
    {
      ...
      uav_waypoint->AddWaypoint (Waypoint (Seconds (0),
                            Vector (i % 3 * 1000 + 500, i / 3 * 1000 + 500, 200)));
    
      uav_waypoint2->AddWaypoint (Waypoint (Seconds (0),
                            Vector (i % 3 * 1000 + 500, i / 3 * 1000 + 500, 200)));
    ...
    for (int i = uav_num/2; i < uav_num; i++)
    {
      ...
      uav_waypoint->AddWaypoint (Waypoint (Seconds (0),
                            Vector ((i-uav_num/2) % 3 * 1000 + 500, (i-uav_num/2) / 3 * 1000 + 500, 200)));
          ...
          if (waypoint_count < waypoint_time[i][j])
            {
              uav_waypoint->AddWaypoint (
                  Waypoint (Seconds (waypoint_time[i][j]),
                            Vector ((i-uav_num/2) % 3 * 1000 + 500, (i-uav_num/2) / 3 * 1000 + 500 + 5000, 200)));
  ...
  MobilityHelper::EnableAsciiAll (ascii.CreateFileStream ("jikken-proposal-1116-mob2.mob"));

  // Animation
  AnimationInterface anim ("jikken-proposal-1116-mob2.xml"); // Mandatory

  for (int i = 0; i < uav_num * 2 + vehicle_num * 2; i++)
    {
      if (i < uav_num / 2)
        {
      ...
      else if (uav_num / 2 <= i && i < uav_num)
        {
          anim.UpdateNodeSize (i, 100, 100);
          anim.UpdateNodeColor (i, 0, 255, 0);
        }
      else if (uav_num <= i && i < uav_num + vehicle_num)
        {
          anim.UpdateNodeSize (i, 100, 100);
          anim.UpdateNodeColor (i, 0, 0, 255);
        }
      else if (uav_num + vehicle_num <= i && i < uav_num + vehicle_num + uav_num / 2)
        {
          anim.UpdateNodeSize (i, 100, 100);
          anim.UpdateNodeColor (i, 255, 0, 0);
        }
      else if (uav_num + vehicle_num + uav_num / 2 <= i && i < uav_num + vehicle_num + uav_num)
        {
          anim.UpdateNodeSize (i, 100, 100);
          anim.UpdateNodeColor (i, 0, 255, 0);
        }
      else if (uav_num + vehicle_num + uav_num <= i &&
               i < uav_num + vehicle_num + uav_num + vehicle_num)
        {
          anim.UpdateNodeSize (i, 100, 100);
          anim.UpdateNodeColor (i, 0, 0, 255);
        }
