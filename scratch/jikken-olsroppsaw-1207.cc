...
{
  //if (mem[m_uid * 10 + m_sender][m_receiver] == false && !(m_sender == m_receiver)) // if this node has never sent a packet
    if (!(m_sender == m_receiver)) // if this node has never sent a packet
    {
      IPv4TestTag tag;
...
TraceSendPacketFromQueue (std::string context, Ipv4Address dst, Ptr<const Packet> packet)
{
  ...
  //std::cout << "(TraceSendPacketFromQueue) " << recv << std::endl; 

  // dtn_testApp(recv, kym, 1, uid);
  dtn_to_olsr(kym, recv, uid);
}
...
olsr_or_dtn (int sender, int recv, int uid)
{
  ...
  for (int i = 0; i < total_node_num; i++)
    {
      NS_LOG_INFO("table_addr[" << i << "] is " << table_addr[i]);
...
sendpacket_interval ()
{
  ...
  {
    for (int j = uav_num; j < total_node_num; j++)
    {
      if (i != j)
      {
  ...
  for (int i = 0; i < total_node_num; i++)
  {
    std::cout << "Node " << i << " 's queuelist is " << queuelist[i].size() << std::endl;
  }
  ...
  for (int i = 0; i < total_node_num; i++)
    {
      ...
      NS_LOG_INFO( "(makeconvarray) convarray[" << olsr_ipv4->GetAddress (1, 0).GetLocal ()
...
main (int argc, char *argv[])
{
  ...
  std::ifstream ifss ("./random-waypoint-mobility-2-3.mob");
  std::string str;
  if (ifss.fail ())
    {
  ...
  for (int u = uav_num; u < uav_num + vehicle_num; u++)
    {
      ...
      for (int v = 0; v < jc[u-uav_num]; v++)
        {
          waypoint->AddWaypoint (
              Waypoint (NanoSeconds (wpl[u-uav_num][v].waypoint_t),
 ...
 */
  ...
  */
  ...
  AnimationInterface anim ("jikken-olsroppsaw-1130.xml");
  //anim.EnablePacketMetadata (true);
  //anim.EnableIpv4RouteTracking ("tinkoko.xml", Seconds (0), Seconds _(appDataEnd), Seconds (0.25));
