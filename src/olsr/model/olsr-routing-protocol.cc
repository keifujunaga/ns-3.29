...
// double temp; //!< original : template of pred
std::map<Ipv4Address, double> temp; //!< original : template of pred
...
RoutingProtocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::olsr::RoutingProtocol")
    ...
    .AddTraceSource ("TraceAddEntry", "substitute pred from main.cc while tracing AddEntry",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_addEntryTrace),
                     "ns3::olsr::RoutingProtocol::TraceaddEntry")
  ;
  return tid;
}
...
/*
void
RoutingProtocol::PrintRoutingTable (Time unit) const
{
  ...
  if (m_hnaRoutingTable->GetNRoutes () > 0)
    {
      std::cout << " HNA Routing Table: ";
      //m_hnaRoutingTable->originalprint(unit);
    }
  else
    {
      std::cout << " HNA Routing Table: empty" << std::endl;
    }
}
*/
                           ...
                           uint32_t distance)
{
  ...
  m_addEntryTrace(m_mainAddress, dest);
  entry.preds = temp;
}
...
/*
void 
RoutingProtocol::setpred (double pred)
{
  temp = pred;
}
*/
...
void
RoutingProtocol::setpred (std::map<Ipv4Address, double> preds)
{
  temp = preds;
}

/*
void RoutingProtocol::originalprint(Time unit)
{
  PrintRoutingTable (unit);
}


void originalprint2(Time unit, Ptr<Node> node)
{
  ...
  for (std::map<Ipv4Address, RoutingTableEntry>::const_iterator iter = m_table.begin (); iter != m_table.end (); iter++)
    {
      std::cout << iter->first << "\t\t";
      std::cout << iter->second.nextAddr << "\t\t";
      if (Names::FindName (ipv4->GetNetDevice (iter->second.interface)) != "")
        {
          std::cout << Names::FindName (ipv4->GetNetDevice (iter->second.interface)) << "\t\t";
        }
      else
        {
          std::cout << iter->second.interface << "\t\t";
...
/*
void
RoutingProtocol::originalprint (Time unit) const
{
  std::cout << "Node: " << m_ipv4->GetObject<Node> ()->GetId ()
      << ", Time: " << unit  
      << ", OLSR Routing table" << std::endl;
