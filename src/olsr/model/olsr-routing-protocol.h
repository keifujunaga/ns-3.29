...
struct RoutingTableEntry
{
  ...
  //double pred; //!< probability of the destination node.
...
public:
   ...
   * 
   */
  //void originalprint(Time unit);
  //void originalprint2(Time unit, Ptr<Node> node, std::map<Ipv4Address, );
...
public:
  ...
  // original
  // void setpred (double pred);
  void setpred (std::map<Ipv4Address, double>);
...
private:
  ...
  //virtual void PrintRoutingTable (Time unit) const;
  ...
  // Pred trace. (original)
  TracedCallback <Ipv4Address, Ipv4Address> m_addEntryTrace;

  typedef void (*TraceaddEntry)
    (Ipv4Address mainAddress, Ipv4Address dest);
