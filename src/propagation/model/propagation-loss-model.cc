                                   ...
                                   Ptr<MobilityModel> b) const
{
  ...
  if (m_next != 0)
    {
      //std::cout << "m_next != 0," << Simulator::Now() << "," << txPowerDbm << "," << self << std::endl;
      self = m_next->CalcRxPower (self, a, b);
    }
  /* zenkai ha kore wo yuukou ni sita 
  if (m_next == 0)
    {
        std::cout << Simulator::Now().GetMicroSeconds() << "," << self << std::endl;
    }
  */
  // std::cout << Simulator::Now() << "," << txPowerDbm << "," << self << std::endl;
  return self;
}
...
{
  // orginal 
  std::cout << "nakagamigamiaaaaaaa" << std::endl;
}
...
RangePropagationLossModel::DoCalcRxPower (double txPowerDbm,
                                          ...
                                          Ptr<MobilityModel> b) const
{
  double orirange = m_range;

  if(a->GetPosition().z == 1.5 && b->GetPosition().z == 1.5)
  {
    orirange = 64;
  }

  if(a->GetPosition().z == 200 && b->GetPosition().z == 200)
  {
    orirange = 1000;
  }

  if((a->GetPosition().z == 200 && b->GetPosition().z == 1.5) || 
     (b->GetPosition().z == 200 && a->GetPosition().z == 1.5))
  {
    orirange = 300;
  }
  ...
  //std::cout << "distance (" << a->GetPosition() << ", " << b->GetPosition() << ")" << " is " << distance << std::endl;

  if (distance <= orirange)
    {
      return txPowerDbm;
    }
  else
