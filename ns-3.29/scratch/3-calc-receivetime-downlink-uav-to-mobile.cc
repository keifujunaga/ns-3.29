...
up_handler (uint32_t i)
{
  ...
  uplink_onoff.Install (all.Get (i + UAV_NUM));
}
...
down_handler ()
{
  ...
  for (int i = UAV_NUM; i < UAV_NUM + MOB_NUM; i++)
    {
      OnOffHelper downlink_onoff ("ns3::UdpSocketFactory",
                         Address (InetSocketAddress (interfaces.GetAddress (i), 80)));
      downlink_onoff.SetConstantRate (DataRate (rate));
      downlink_onoff.SetAttribute ("PacketSize", UintegerValue (packetSize));
      downlink_apps = downlink_onoff.Install (all.Get (0));
      downlink_apps.Start (Seconds (APPSTARTSEC));
      downlink_apps.Stop (Seconds (APPSTARTSEC+5.0));
    }
}
...
main (int argc, char *argv[])
{
  ...
  mobility.SetPositionAllocator ("ns3::GridPositionAllocator", "MinX", DoubleValue (0), "MinY",
                                 DoubleValue (0), "Z", DoubleValue (0), "DeltaX", DoubleValue (5.0),
                                 "DeltaY", DoubleValue (5.0), "GridWidth", UintegerValue (7),
  ...
  devices = wifi.Install (wifiPhy, wifiMac, all);
  ...
  for (int i = 0; i < MOB_NUM; i++)
    {
      apps_sink = sink.Install (mob.Get (i));
      apps_sink.Start (Seconds (APPSTARTSEC));
      apps_sink.Stop (Seconds (APPSTOPSEC));
    }

  /// set onoff node
  Simulator::Schedule (Seconds (329), &down_handler);
