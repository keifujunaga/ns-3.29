...
main (int argc, char *argv[])
{
  ...
  */
  ...
  apps_sink6.Start (Seconds (0.0));                       Address (Ipv4Address("255.255.255.255"), 80));
  ...
  */
  ...
  ApplicationContainer apps_sink = sink.Install (mobilenode);
  apps_sink.Start (Seconds (0.0));
  apps_sink.Stop (Seconds (appTotalTime));
  ...
  for (uint32_t i = 0; i < nodeContainer.GetN (); ++i)
    {
      if (i < 30)
        {
          anim.UpdateNodeDescription (nodeContainer.Get (i), "UAV"); // Optional
          anim.UpdateNodeColor (nodeContainer.Get (i), 255, 0, 0); // Optional
        }
      else
        {
          anim.UpdateNodeDescription (nodeContainer.Get (i), "mob"); // Optional
          anim.UpdateNodeColor (nodeContainer.Get (i), 0, 255, 0); // Optional
        }
    }

  anim.EnablePacketMetadata (); // Optional
  anim.EnableIpv4RouteTracking ("routingtable-wireless.xml", Seconds (0), Seconds (600),
                                Seconds (1.0)); //Optional
  ...
  */
  ...
  // wifiPhy.EnablePcapAll("dicomo-jikken-olsr", true);
  // wifiPhy.EnableAsciiAll(ascii.CreateFileStream("mythird2.tr"));

  AnimationInterface anim ("wireless-animation.xml"); // Mandatory
  
  anim.EnableIpv4RouteTracking ("dicomo-jikken-olsr6-routingtable.xml", Seconds (100), Seconds (200),  Seconds (0.5)); //Optional
