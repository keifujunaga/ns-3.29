...
MyApp::SendPacket (void)
{
  //if (mem[m_uid * 10 + m_sender] == false) // if this node has never sent a packet
    //{
      ...
      NS_LOG_INFO("(MyApp::SendPacket) Time: " << Simulator::Now().GetSeconds() << " send packet " << packet->GetUid() << " is started");
...
TraceAddEntry (std::string context, Ipv4Address mainAddress, Ipv4Address dest)
{
  ...
  olsrh.setpred (olsrContainer, kym,
                 mainmap[convarray[dest]][convarray[Ipv4Address ("192.168.1.5")]]);
}
...
main (int argc, char *argv[])
{
  ...
  waypoint2->AddWaypoint (Waypoint(Seconds(60), Vector(1050, 0, 1.5)));
  ...
  o_waypoint2->AddWaypoint (Waypoint(Seconds(60), Vector(1050, 0, 1.5)));
