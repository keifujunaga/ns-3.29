...
public:
   ...
   *  Original
   */
  void Drop (NodeContainer c, int uid);
  void SendJudge(NodeContainer c, int snodenum, Ptr<const Packet> packet, bool judge);
  void setOpp_preds(NodeContainer c, int snodenum, std::map<Ipv4Address, double> main_preds);
  void setOpp_preds2(NodeContainer c, int snodenum, std::map<Ipv4Address, double> main_preds);


private:
  /** The factory to create Epidemic  GetRoutingTable (Ptr<Node> node, Ipv4Address addresses[]);routing object */
