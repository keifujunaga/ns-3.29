...
RoutingProtocol::BroadcastPacket (Ptr<Packet> p)
{
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin ();
       j != m_socketAddresses.end (); ++j)
    {
      ...
      NS_LOG_WARN ("m_mainAddress " << m_mainAddress << ", Packet " << p << " is sent to" << destination);
...
{
  NS_LOG_WARN (this << dst << queueEntry.GetPacketID ()); // function to warn
  ...
  if (m_mainAddress == Ipv4Address("192.168.2.5"))
  {
    preds[Ipv4Address("192.168.2.5")] = 1;
  }

  if (dst == Ipv4Address("192.168.2.5"))
  {
    opp_preds2[Ipv4Address("192.168.2.5")] = 1;
  }
  

  NS_LOG_WARN ("Time : " << Simulator::Now ().GetSeconds () << "packet uid is " << p->GetUid() << ", packet size is " << p->GetSize() << ", main address is " << m_mainAddress << ", dst is " << dst);

  NS_LOG_WARN("Time : " << Simulator::Now ().GetSeconds () << ", preds["
              << header.GetDestination () << "] = " << preds[header.GetDestination ()] << ", "
              << "opp_preds2[" << header.GetDestination ()
              << "] = " << opp_preds2[header.GetDestination ()]);

    if (preds[header.GetDestination ()] < opp_preds2[header.GetDestination ()])
    {
...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_WARN ("m_mainAddress " << m_mainAddress << " is sent beacons");
                  ...
                  if (m_queue.Find (current_Header.GetPacketID ()).GetPacketID () == 0)
                    {
                      ...
                      {
                        if (p->GetSize() >= 200)
                          NS_LOG_WARN("(RouteInput) packet " << p->GetUid() << " ,packet size " << p->GetSize() << " is entry");
...
{
  NS_LOG_WARN (this << m_mainAddress<< ", " << dest << ", " << firstNode);
  // Creating the packet
...
RoutingProtocol::RecvEpidemic (Ptr<Socket> socket)
{
  ...
  if (tHeader.GetMessageType () == TypeHeader::BEACON)
    {
      NS_LOG_WARN ("Got a beacon from " << sender << " " << packet->GetUid () << " " << m_mainAddress);
    ...
    {
      NS_LOG_WARN ("Got a A reply from " << sender << " " << packet->GetUid () << " "
                                          << m_mainAddress);
        SummaryVectorHeader packet_SMV;
        packet->RemoveHeader (packet_SMV);
        SendDisjointPackets (packet_SMV, sender);
    ...
    {
      NS_LOG_WARN ("Got a A reply back from " << sender << " " << packet->GetUid () << " " << m_mainAddress);
        SummaryVectorHeader packet_SMV;
        packet->RemoveHeader (packet_SMV);
        SendDisjointPackets (packet_SMV, sender);
    }
      ...
      if (pNew > pOld)
        {
          ...
          preds[itr->first] = pNew;
            NS_LOG_WARN (Simulator::Now ().GetSeconds ()
               << ", " << m_mainAddress << ", " << itr->first << ", " << preds[itr->first]);
