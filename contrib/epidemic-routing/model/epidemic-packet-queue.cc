...
bool
PacketQueue::Enqueue (QueueEntry & entry, Ipv4Address mainAddress)
{
  ...
  {
    temp = itr->second.GetPacket()->GetSize();
    count = count + temp;
    //NS_LOG_WARN(itr->second.GetPacket()->GetUid() << ", " << itr->second.GetPacket()->GetSize());
  }
  //NS_LOG_WARN("(PacketQueue::Enqueue)" << "Time: " << Simulator::Now().GetSeconds() <<  "mainAddress is " << mainAddress 
  //<< " totalpacket is " << count);
  NS_LOG_WARN("(PacketQueue::Enqueue), "  << Simulator::Now().GetSeconds() <<  ", " << mainAddress 
  << ", " << count);
  //
  Purge (true);
  return true;
...
PacketQueue::Purge (bool outdated /* = false */)
{
  ...
  int count = 0; int temp = 0; 
  //int maxbuf = 10000000; // 10Mbytes // 100000 10kBytes
  int maxbuf = 100000;
  for (auto i = m_map.begin(); i != m_map.end(); ++i)
  {
    temp = i->second.GetPacket()->GetSize();
    count = count + temp;
  }

  if (outdated && (m_map.size () > m_maxLen || count > maxbuf))
    {
